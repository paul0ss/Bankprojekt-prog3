package bankprojelt.oberflaeche;
...
public class KontoOberflaeche extends BorderPane {
	 ...
	 */
	public KontoOberflaeche(KontoController c, Konto k)
	{	
		this.k = k;
		this.controller = c;
		...
		stand.textProperty().bind(k.kontostandProperty().asString());
		if(k.getImPlusProperty().get() == false) {
			stand.setFill(Color.RED);
		}else {
			stand.setFill(Color.GREEN);
		}
		k.getImPlusProperty().addListener(e -> {
			if(k.getImPlusProperty().get() == false) {
			stand.setFill(Color.RED);
		}else {
			stand.setFill(Color.GREEN);
		...
		System.out.println(k.getImPlus());
		...
		if(k.getGesperrt().get()) {
			gesperrt.setSelected(true);
		}else {
			gesperrt.setSelected(false);
		}
		gesperrt.selectedProperty().addListener(e -> {
		...
		gesperrt.textProperty().bind(k.getGesperrt().asString());
		...
		adresse.textProperty().bindBidirectional(k.getInhaber().getAdresseProperty());
